#!/usr/bin/env python3
"""
SnakeByte CJK - Enhanced Binary Viewer with CJK/Japanese Support

A powerful terminal-based hex viewer specifically designed for analyzing binary files with CJK character support,
...
class BinaryFileViewer:
    ...
    This version includes enhanced support for CJK (Chinese/Japanese/Korean) character sets.
    """
    ...
    def __init__(self, filename):
        ...
        # Encoding support with CJK encodings prioritized
        self.encodings = [
            'ascii',
            'shift_jis',  # Shift-JIS for Japanese
            'utf-8',
            'latin-1',    # ISO-8859-1
            'cp1252',     # Windows-1252
        ...
        # Print locale and encoding information for debugging
    ...
    def display_printable(self, char_byte):
        ...
        Returns (character, is_error, is_cjk) tuple
        """
        try:
            # First decode using the file's encoding
            char = char_byte.decode(self.encoding)
            
            # For terminal display, we need to ensure the character is compatible with
            # the terminal's encoding (usually UTF-8 for Linux terminals)
            if ord(char) > 127:  # Non-ASCII character
                # We've successfully decoded it, mark it as a CJK character
                return char, False, True  # char, is_error, is_cjk
            elif char in string.printable and char not in '\t\n\r\v\f':
                return char, False, False  # Normal printable ASCII
            return '.', False, False  # Not printable, but not an encoding error
    ...
    def format_line(self, offset, data):
        ...
        Returns (line_string, error_positions, cjk_positions) tuple
        """
        # Address column
        line = f"{offset:08x}: "
        ...
        cjk_positions = []    # Track positions with CJK characters
        
        for i, byte in enumerate(display_data):
            char, is_error, is_cjk = self.display_printable(bytes([byte]))
            char_section += char
            if is_error:
                error_positions.append(i)  # Store position of encoding error
            if is_cjk:
                cjk_positions.append(i)    # Store position of CJK character
        ...
        return result, error_positions, cjk_positions
    ...
    def run(self, stdscr=None):
        ...
        try:
            if curses.has_colors():
                ...
                if self.color_scheme == 1:
                    ...
                    curses.init_pair(6, curses.COLOR_BLUE, curses.COLOR_WHITE)      # CJK characters
                # For dark themes
                    ...
                    curses.init_pair(4, curses.COLOR_BLACK, curses.COLOR_GREEN)     # Info line
                    curses.init_pair(5, curses.COLOR_WHITE, curses.COLOR_RED)       # Encoding error
                    curses.init_pair(6, curses.COLOR_CYAN, curses.COLOR_BLACK)      # CJK characters
        ...
        content_height = max_y - 3  # Reserve last two lines for status and help/info
        ...
        while running:
            try:
                ...
                for i in range(content_height):
                    ...
                    line, error_positions, cjk_positions = self.format_line(offset, data)
                    ...
                    if is_current_line:
                        try:
                            ...
                            if self.current_offset < self.file_size:
                                char, is_error, is_cjk = self.display_printable(bytes([self.file_content[self.current_offset]]))
                                if self.has_colors:
                                    if is_cjk:
                                        stdscr.addstr(i, char_pos, char, curses.color_pair(6) | curses.A_BOLD)
                                    else:
                    ...
                    for j in range(min(self.bytes_per_line, len(data))):
                        if (i, j) in search_positions:
                            try:
                                ...
                                # Highlight the character representation
                                char, is_error, is_cjk = self.display_printable(bytes([data[j]]))
                                if self.has_colors:
                                    # Use special color for CJK chars
                                    if is_cjk:
                                        stdscr.addstr(i, char_pos, char, curses.color_pair(6) | curses.A_BOLD)
                                    else:
                    ...
                    # Highlight CJK characters
                    for j in cjk_positions:
                        if j < len(data):
                            try:
                                # Calculate position in the character representation section
                                char_start = hex_start + 3 * self.bytes_per_line + 3
                                ...
                                char, _, _ = self.display_printable(bytes([data[j]]))
                                
                                # Only highlight if it's not already highlighted for another reason
                                if not is_current_line and (i, j) not in search_positions:
                                    if self.has_colors:
                                        stdscr.addstr(i, char_pos, char, curses.color_pair(6) | curses.A_BOLD)
                                    else:
                                        stdscr.addstr(i, char_pos, char, curses.A_BOLD)
                            except curses.error:
                                pass
                
                # Status line
                status = f" File: {self.filename} | Size: {self.file_size} bytes | "
                status += f"Offset: {self.current_offset}/{self.file_size} ({self.current_offset/self.file_size:.1%}) | "
                
                if self.display_shift != 0:
                    status += f"Shift: {self.display_shift} bytes | "
                
                # Show endianness and encoding
                status += f"Endian: {self.endian} | "
                status += f"Encoding: {self.encoding} | "
                
                if self.search_results:
                    status += f"Search: {self.current_search_idx + 1}/{len(self.search_results)} | "
                
                if show_help:
                    status = "h:Hide help | q:Quit | Arrow:Navigate | s:Search | j:Jump | </> Endian | e:Encoding | [/] Shift | i:Investigate | v:Values | c:Color"
                else:
                    status += "h:Help | q:Quit | s:Search | j:Jump | i:Investigate | e:Encoding | c:Color"
                
                # Display status line
                try:
                    if self.has_colors:
                        stdscr.addstr(max_y - 2, 0, status[:max_x-1], curses.color_pair(1))
                    else:
                        stdscr.addstr(max_y - 2, 0, status[:max_x-1], curses.A_REVERSE)
                except curses.error:
                    try:
                ...
                # Show investigation info or help on second-to-last line
                if show_investigate and self.search_results and self.current_search_idx >= 0:
                    ...
                    try:
                        help_info = (
                            "Arrow keys: Navigate | Home/End: Start/End | PgUp/PgDown: Page navigation | "
                            "s: Search | n/p: Next/prev search result | j: Jump to offset | "
                            "[/]: Shift display ±1 byte | {/}: Shift ±4 bytes | \\: Reset shift | "
                            "</> Toggle endianness | e: Cycle encoding | v: Toggle value display | i: Investigate hit | c: Cycle color scheme"
                        )
                        if self.has_colors:
                    ...
                    # Clear second-to-bottom line when not showing info
                    try:
...
def main():
    """Main entry point for SnakeByte CJK"""
    # Print locale and encoding information for debugging
    print(f"Current locale: {locale.getlocale()}")
    print(f"Default encoding: {sys.getdefaultencoding()}")
    
    parser = argparse.ArgumentParser(description="SnakeByte CJK - Enhanced Binary File Viewer with CJK Support")
    ...
    parser.add_argument("--encoding", "-e", help="Initial encoding to use (default: ascii)")
    ...
    try:
        ...
        # Set initial encoding if specified
        if args.encoding:
            if args.encoding in viewer.encodings:
                viewer.encoding = args.encoding
                viewer.encoding_idx = viewer.encodings.index(args.encoding)
            else:
                print(f"Warning: Encoding '{args.encoding}' not recognized. Adding to encodings list.")
                viewer.encodings.append(args.encoding)
                viewer.encoding = args.encoding
                viewer.encoding_idx = len(viewer.encodings) - 1
